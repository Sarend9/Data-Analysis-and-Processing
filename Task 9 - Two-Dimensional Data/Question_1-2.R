...
#         Для ОПИСАНИЯ и УДАЛЕНИЯ наблюдений нам нужно знать их индексы (номера строк).
          cat("На графике нажмите на точки-выбросы, после чего нажмите ESC \n")
     
          identify <- identify(x1); identify  # интерактивное определение индексов точек
          cat("Номера выбросов: ", identify, "\n")
...
#         в которой эти две переменные изменяются по отношению друг к другу.
          ...
          Cov_x.y(x1.clean) 
          cat("Ковариация Cov: ", Cov_x.y(x1.clean), "\n")
...
#                            N                     N σx σy           |   среднее      значение ρ Пирсона = "0"  (отсутсвие линейной корреляции).
          ...
          P_x.y(x1.clean)
          cat("коэффициент p: ", P_x.y(x1.clean), "\n")
          P_x.y(x1)
          cat("коэффициент p при неудаленных выбросах: ",P_x.y(x1), "\n")
